import os
import argparse
from slot_abstractor_svrt import *
import time
import datetime
...
parser.add_argument('--model_name', type=str, default='slot_attention_augmentations_first_more_pretrained_svrt_alltasks_frozen_autoencoder_abstractor_scoring')
...
slot_model = load_slot_checkpoint(slot_model,args.model_checkpoint)
...
for epoch in range(1,args.num_epochs+1):
	...
	for batch_idx, (img,target) in enumerate(train_dataloader):
		...
		score = abstractor_model(feat_slots,pos_slots,device)
...
# 		del recons, masks, slots
	...
	# 		# print(torch.max(img), torch.min(img),img.shape)
		...
		score = abstractor_model(val_feat_slots,val_pos_slots,device)
			# print("scores and target>>",scores,target)
		pred = torch.round(sigmoid_activation(score)).int()
	...
	# 		# print(torch.max(img), torch.min(img),img.shape)
			...
			score = abstractor_model(test_feat_slots,test_pos_slots,device)
			# print("scores and target>>",scores,target)
			pred = torch.round(sigmoid_activation(score)).int()
...
model_dir = model_dir + str(2*args.n) + '/'
check_path(model_dir)
